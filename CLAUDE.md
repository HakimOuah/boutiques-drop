...
- Recherche produit, trois skills étanches : `ideation-produit` (TrendTrack + idées) · `recherche-mots-cles` (SEMrush + SERP + sonde prix) · `sourcing-aliexpress` (après GO marché). Chaîne 5 phases : orchestrateur `/recherche-produit`. Boucle `/chasse-clusters`, qualification express `/qualifie-idees` — sous-agents phase0→phase5 dans `.claude/agents/`.
