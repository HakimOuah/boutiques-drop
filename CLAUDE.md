...
- **Portage des skills vers Hermes : `scripts/porter-skills-hermes.py`** (31/08/2026). La source de vérité reste `.claude/skills/<nom>/SKILL.md` — un seul endroit où une règle se modifie. Le script en dérive la version Hermes dans `~/.hermes/skills/oh-ventures/`, en ajoutant le frontmatter attendu et un bloc d'exécution (outils requis, chemins absolus, chargement des `.env`). **Relancer le script après toute modification d'un skill** : sans ça, Hermes travaille sur une version périmée. Leçon tirée de `GROK-BOT-FLEET.md`, où les instructions recopiées à la main ne se propageaient pas.
