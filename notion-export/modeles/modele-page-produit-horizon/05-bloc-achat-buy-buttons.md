# 🛒 5. Bloc d'achat — buy-buttons.liquid

- **URL Notion** : https://app.notion.com/p/3a11f38c315481739b63d8d9971d11b7
...
- **Nom** : `blocks/buy-buttons.liquid`
- **Chemin local** : `boutique-pipeline/docs/horizon-product-page-reference/blocks/buy-buttons.liquid`

> ⚠️ **Ce fichier dépend de composants internes au thème Horizon et ne peut pas être copié seul dans n'importe quel thème.** Il utilise des Web Components, des `snippets`, des `content_for 'block'` et des clés de traduction (`t:`) propres à Horizon.

## Éléments documentés

- **Formulaire produit** : `product-form-component` + `{% form 'product' %}`, champ caché `id` = variante sélectionnée (`ref="variantId"`).
- **Variante sélectionnée** : `closest.product.selected_or_first_available_variant`.
- **Gestion du stock** : calcul `can_add_to_cart` selon `inventory_quantity`, `inventory_policy` (`deny`) et `inventory_management == 'shopify'` ; libellés `add_to_cart` / `sold_out` / `unavailable`.
- **Règles de quantité** : `variant.quantity_rule` (min, increment, max) → affichage `quantity-label`, `quantity-rules`.
- **Prix par volume** : `volume-pricing` si `product.quantity_price_breaks_configured?`, avec table repliable au-delà de 2 paliers.
- **Bouton Ajouter au panier** : via `content_for 'block', type: 'add-to-cart'`.
- **Disponibilité en retrait** : `local-pickup` (activable par `show_pickup_availability`), lit `store_availabilities` filtrées sur `pick_up_enabled`.
- **Tiroir de retrait** : `theme-drawer` (`pickup-drawer-{{ block.id }}`) listant chaque point de retrait, adresse et téléphone.
- **Styles et snippets dépendants** : `theme-drawer-styles`, `buy-buttons-styles`, `spacing-style`, `gift-card-recipient-form`, icônes SVG inline (`icon-error.svg`, `icon-available.svg`, `icon-unavailable.svg`, `icon-close.svg`).
- **Paiement accéléré retiré du rendu actuel** : le bloc `dynamic-checkout` / boutons de paiement accéléré n'est pas rendu dans cette version (retiré volontairement).

## Valeurs / dépendances à adapter

- Clés de traduction `t:` → présentes dans les fichiers de locales Horizon.
- Web Components (`product-form-component`, `volume-pricing`, `local-pickup`, `theme-drawer`) → fournis par Horizon.
- À reconstruire selon les schémas du thème cible si thème différent.

## Code complet — Partie 1/2 (Template Liquid)
...
## Code complet — Partie 2/2 (Schema)
...
- [ ] Web Components et snippets Horizon présents dans le thème cible (sinon reconstruire)
- [ ] Clés de traduction `t:` disponibles dans les locales
- [ ] Gestion du stock testée (disponible / épuisé / indisponible)
- [ ] Règles de quantité et prix par volume testés
- [ ] Retrait en magasin configuré ou désactivé
- [ ] Décision assumée sur le paiement accéléré (retiré ici)
