# Décisions et états observés au 24 août 2026

Historique à consulter uniquement pour le magasin concerné. Vérifier l’état actuel ; aucune coordonnée ne doit être copiée sur une autre boutique par défaut.
...
- Orysbain : aller au bout du test. Lumière Matière : domaine **lumierematiere.fr** (le `.com` existe aussi, identité = `.fr`), **Workspace en cours le 24/08**. E-mail boutique = `contact@lumierematiere.fr` (utilisateur Workspace), jamais le Gmail gratuit servant d'admin. Profil Chrome `Lumière Matière` connecté à cet utilisateur seulement. Téléphone affiché = ~~`+33 7 56 82 80 94`~~ → **`+33 7 56 91 60 84`, ligne propre à la boutique** (rectifié le 31/08 par Hakim, réglages Shopify à l'appui, reconfirmé le 05/09).
- Téléphone **affiché** sur le site = `+33 7 56 82 80 94` **pour les quatre boutiques du parc**. Un n° brûlé pour *créer un compte Google* reste utilisable en footer / SAV s'il décroche.
- **Ce n'est pas une valeur universelle.** La source d'une boutique est le champ **Téléphone de ses réglages Shopify** ; on aligne le site dessus, jamais l'inverse. Lumière Matière a sa propre ligne, `+33 7 56 91 60 84` — s'y fier plutôt qu'à cette page a déjà fait proposer deux fois une « correction » qui aurait cassé la boutique (31/08, puis 05/09).
...
Le GMC du parc naît de l'app Shopify **Google & YouTube**, liée à Ads **sans campagne**. Flux déjà validé = on le laisse vivre. Compte Google / Workspace **quand on peut le vérifier** ; pas le jour du mur SMS. L'e-mail boutique (IONOS ou Workspace) doit déjà être celui du domaine.
