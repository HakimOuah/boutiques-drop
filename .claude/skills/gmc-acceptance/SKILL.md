...
- Orysbain : aller au bout du test. Lumière Matière : Hakim a choisi d'ouvrir le site (24/08 soir). Domaine acheté. **Workspace reporté** : les n° `06 77 70 08 10` et `07 56 82 80 94` sont brûlés pour la vérif SMS Google (« trop de comptes ») ; le n° OH neuf (VoIP) ne reçoit pas les short codes. Boîte **IONOS** `contact@lumierematiere.fr` maintenant. Workspace plus tard = **même adresse**, bascule MX — on ne « rattache » pas IONOS à un Gmail gratuit. Pas de Gmail `contact.lumierematiere@gmail.com`.
