# Recette Terry — enseignement à contextualiser

Les jours, paliers, AOV et budgets ci-dessous sont des heuristiques historiques, jamais une autorisation ni des seuils universels. Le protocole de test courant et l’économie de commande priment.
...
PMAX est le moteur central : tous les canaux Google (Search, Shopping, YouTube, Display, Discover, Gmail), optimisation temps réel des enchères, placements, créas et audiences. Hypothèse du formateur : il peut dépasser des structures manuelles lorsque il est nourri de données de conversion propres et d'un budget suffisant. Pour l'e-commerce et le dropshipping, il capture ET étend la demande.
...
- **1 seule campagne PMAX**, feed des produits autorisés et conformes ; la structure dépend du test validé. Concentrer la donnée accélère l'apprentissage.
...
Métrique de décision : contribution après coûts produit, livraison, paiement, retours, autres coûts variables et publicité, avec TVA et devise cohérentes. Le ROAS donne du contexte ; le profit tranche. Vérifier la fiabilité du tracking (outil type TrackBee) avant toute décision — ne pas faire confiance aveuglément aux analytics Shopify.
