...
**Reconfirmé le 15/08/2026 (Noirmont, thème travail 205451100498)** : le faux négatif `upsertedThemeFiles: []` s'est reproduit sur un lot de 3 fichiers passés en `body:{type:URL}` — écriture pourtant réussie, prouvée par empreinte **et** diff octet par octet du contenu redescendu. Recette staged upload validée de bout en bout : `stagedUploadsCreate` (POST) → `curl -F` avec les `parameters` renvoyés + `file=@…` en dernier → `themeFilesUpsert` avec le `resourceUrl`. C'est la voie par défaut pour tout `templates/*.json` volumineux : retaper le corps en `TEXT` dans l'appel est le vrai risque d'infidélité, pas l'API.
