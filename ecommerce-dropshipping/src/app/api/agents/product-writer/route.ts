...
import { generateProductPage } from "@/lib/agents/product-writer";
...
export async function POST(req: NextRequest) {
  ...
  const { productId, productName, productDescription, nicheKeywords } = await req.json();
  ...
  (async () => {
    try {
      const result = await generateProductPage(
        { productId, productName, productDescription, nicheKeywords },
        (msg: string) => {
          writer.write(
